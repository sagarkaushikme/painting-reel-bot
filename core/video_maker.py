...
- Painting zoom reels (zoompan on downloaded image)
- Diary/ledger reels (old paper background + handwriting text + ASMR sounds)
"""
...
# ============================================================
# PAINTING REEL CREATOR (existing logic, unchanged)
# ============================================================
...
# ============================================================
# DIARY/LEDGER REEL CREATOR (new — for paisa_ka_gyaan)
# ============================================================


def create_diary_reel(zoom_data: dict, config: dict, output_path: str) -> str:
    """
    Creates a diary/ledger style reel.
    Each scene = old paper background + animated handwriting text
    + corresponding ASMR sound.

    Args:
        zoom_data: Analysis result with zoom_sequence
        config: Page config.yaml data
    ...
    BACKGROUND_IMAGE = os.path.join(base_dir, "assets", "backgrounds", "old_paper_texture.jpg")
    font_family = config.get("font_family", "Kalam-Regular.ttf")
    FONT_PATH = os.path.join(base_dir, "assets", "fonts", "handwriting", font_family)

    SOUND_MAP = {
        "book_open": os.path.join(base_dir, "sounds", "book_open.mp3"),
        "writing": os.path.join(base_dir, "sounds", "pen_writing.mp3"),
        "calculating": os.path.join(base_dir, "sounds", "calculator_click.mp3"),
        "stamping": os.path.join(base_dir, "sounds", "stamp_thud.mp3"),
        "book_close": os.path.join(base_dir, "sounds", "book_close.mp3"),
    }

    # Check if background image exists
    if not os.path.exists(BACKGROUND_IMAGE):
        logger.warning(f"⚠️ Background image not found: {BACKGROUND_IMAGE}")
        logger.info("Creating a placeholder old paper texture...")
        _create_placeholder_background(BACKGROUND_IMAGE)

    # Check if font exists, fallback if not
    if not os.path.exists(FONT_PATH):
        logger.warning(f"⚠️ Font not found: {FONT_PATH}, using system fallback")
    ...
    logger.info(f"📖 Creating diary reel with {len(sequence)} scenes...")

    segments = []

    for i, scene in enumerate(sequence):
        text = scene.get("text", "")
        scene_type = scene.get("scene_type", "writing")
        zoom = scene.get("zoom_level", 1.0)

        segment_path = f"/tmp/diary_seg_{i}.mp4"

        text_pos = scene.get("text_position", "center")

        # Prepare text file for drawtext (handles unicode/special chars safely)
        text_file_path = f"/tmp/diary_text_{i}.txt"
        wrapped_text = ""
        fontsize = 75
        ...
        if text.strip():
            ...
            # Auto-calculate duration: 2.0 sec per word + 3 sec buffer (min 10 sec)
            # This gives every user enough time to comfortably read the text
            duration = max(10, round(word_count * 2.0 + 3))
        else:
            # No text → clean closing scene (5 sec)
            duration = 5
        ...
        if wrapped_text.strip():
            ...
            vf_parts.append(
                ...
                f"x=(w-text_w)/2:"
                f"y={text_y}:"
                f"alpha='if(lt(t,0.5),t/0.5,1)':"  # 0.5 second fade-in — text visible sooner
                f"line_spacing=20"
            )
        ...
        sound_path = SOUND_MAP.get(scene_type)
        has_sound = sound_path and os.path.exists(sound_path)
        if has_sound:
        ...
        if has_sound:
            ...
            # No audio — generate silent audio track for concat compatibility
            cmd.extend([
                "-f", "lavfi", "-i", f"anullsrc=r=44100:cl=stereo",
                "-c:a", "aac",
                "-shortest",
            ])
            # Note: when using anullsrc, we need to reorder inputs
            ...
            segments.append(segment_path)
            logger.info(
                f"  ✅ Diary scene {i + 1}/{len(sequence)} done "
                f"({duration}s, type={scene_type})"
            )
            continue
        ...
        if result.returncode != 0:
            ...
            logger.error(f"❌ Diary segment {i} failed: {error_msg}")
            # Try without sound as fallback
            cmd_nosound = [
                "ffmpeg", "-y",
                "-loop", "1",
                "-i", BACKGROUND_IMAGE,
            ...
            subprocess.run(cmd_nosound, capture_output=True, text=True, timeout=120)
    ...
    if os.path.exists(output_path):
        size_mb = os.path.getsize(output_path) / (1024 * 1024)
        logger.info(f"📖 Final diary reel: {output_path} ({size_mb:.1f}MB)")
    else:
        raise RuntimeError(f"Output file not created: {output_path}")
...
# ============================================================
# INDIVIDUAL SEGMENT CREATION (painting zoom mode)
# ============================================================
...
def _create_placeholder_background(output_path: str):
    """
    Create a simple old-paper-like placeholder background image.
    This is a fallback — the real texture should be downloaded separately.
    """
    try:
        ...
        # Create a warm, paper-like background
        img = Image.new("RGB", (1080, 1920), color=(245, 235, 220))
        ...
        logger.info(f"📄 Created placeholder background: {output_path}")
